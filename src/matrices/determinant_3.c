...
/*   Updated: 2025/08/06 22:08:05 by abessa-m         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "minirt.h"

static t_m2		m3_submatrix(t_m3 m3, t_cnv rm);
static float	m3_cofactor(t_m3 m3, t_cnv c);
//static float	m3_minor(t_m3 m3, t_cnv c);

float	m3_determinant(t_m3 m3)
{
	float	ret;

	ret = m3.m[0][0] * m3_cofactor(m3, (t_cnv){0, 0})
		+ m3.m[0][1] * m3_cofactor(m3, (t_cnv){0, 1})
		+ m3.m[0][2] * m3_cofactor(m3, (t_cnv){0, 2});
	return (ret);
}
...
/*int	main(void)
{
	...
	if (m3_equal(m3, m4_submatrix(m4, (t_cnv){2, 1})))
		ft_printf("m4 submatrix:   Correct\n");
	else
		ft_printf("m4 submatrix:   Incorrect\n");
	m3 = (t_m3){{{1, 5, 0}, {-3, 2, 7}, {0, 6, -3}}};
	m2 = (t_m2){{{-3, 2}, {0, 6}}};
	if (m2_equal(m2, m3_submatrix(m3, (t_cnv){0, 2})))
		ft_printf("m3 submatrix:   Correct\n");
	else
		ft_printf("m3 submatrix:   Incorrect\n");
	m3 = (t_m3){{{3, 5, 0}, {2, -1, -7}, {6, -1, 5}}};
	if ((fl_equal(m2_determinant(m3_submatrix(m3, (t_cnv){1, 0})),
			m3_minor(m3, (t_cnv){1, 0})))
			&& (m3_minor(m3, (t_cnv){1, 0}) == 25.0f))
		ft_printf("m3 minor:       Correct\n");
	else
		ft_printf("m3 minor:       Incorrect\n");
	if ((m3_minor(m3, (t_cnv){0, 0}) == -12.f)
		&& (m3_cofactor(m3, (t_cnv){0, 0}) == -12.f))
		ft_printf("m3 cofactor 1:  Correct\n");
	else
		ft_printf("m3 cofactor 1:  Incorrect\n");
	if ((fl_equal(m3_minor(m3, (t_cnv){1, 0}), 25.f))
		&& (fl_equal(m3_cofactor(m3, (t_cnv){1, 0}), -25.f)))
		ft_printf("m3 cofactor 2:  Correct\n");
	else
		ft_printf("m3 cofactor 2:  Incorrect\n");
	m3 = (t_m3){{{1, 2, 6}, {-5, 8, -4}, {2, 6, 4}}};
	if (fl_equal(m3_cofactor(m3, (t_cnv){0, 0}), 56)
		&& fl_equal(m3_cofactor(m3, (t_cnv){0, 1}), 12)
		&& fl_equal(m3_cofactor(m3, (t_cnv){0, 2}), -46))
	...
	if ((fl_equal(m4_cofactor(m4, (t_cnv){0, 0}), 690))
		&& (fl_equal(m4_cofactor(m4, (t_cnv){0, 1}), 447))
		&& (fl_equal(m4_cofactor(m4, (t_cnv){0, 2}), 210))
		&& (fl_equal(m4_cofactor(m4, (t_cnv){0, 3}), 51)))
