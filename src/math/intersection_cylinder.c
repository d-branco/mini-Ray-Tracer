...
/*   Updated: 2025/08/22 16:44:03 by abessa-m         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	smll_dst_to_circle(
				t_tuple pnt, t_tuple dir, t_lst_obj *circle, float *dst);
...
int	cy_top_xs(t_scene *rt, t_tuple pnt, t_tuple dir, t_lst_obj *cy)
{
	...
	if (smll_dst_to_circle(pnt, dir, &circle, &dst))
		return (TRUE);
	circle.center = cy->center;
	circle.center.x -= cy->vec_uni.x * (cy->height / 2.0f);
	circle.center.y -= cy->vec_uni.y * (cy->height / 2.0f);
...
int	smll_dst_to_cy_top(t_scene *rt, t_tuple dir, t_lst_obj *cy, float *dst)
{
	...
	circle.center.y += cy->vec_uni.y * (cy->height / 2.0f);
	circle.center.z += cy->vec_uni.z * (cy->height / 2.0f);
	if (smll_dst_to_circle(rt->c_coo, dir, &circle, dst))
		ret = TRUE;
	circle.center = cy->center;
	circle.center.x -= cy->vec_uni.x * (cy->height / 2.0f);
	circle.center.y -= cy->vec_uni.y * (cy->height / 2.0f);
...
static int	smll_dst_to_circle(
				t_tuple tpl, t_tuple dir, t_lst_obj *circle, float *dst)
{
	t_tuple		oc;
	float		denom;
	float		t;
	...
	oc = pnt_subtr_to_vec(tpl, circle->center);
	...
	if (t <= EPSILON || t >= *dst)
		return (FALSE);
	xs_pnt = vec_addiction(tpl, vec_scalar_multiplication(t, dir));
